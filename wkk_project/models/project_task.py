...
class ProjectTask(models.Model):
    ...
    @api.depends("project_id", "project_stage_id", "invisible_project_stage_ids", "project_id.is_template")
    def _compute_active(self):
        for task in self:
            task.active = not (
                not task.project_id.is_template
                and task.project_stage_id
                and task.invisible_project_stage_ids
                and task.project_stage_id in task.invisible_project_stage_ids
            )
